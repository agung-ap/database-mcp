...
func NewManager(connections []config.Connection) (*Manager, error) {
	...
	for _, conn := range connections {
		...
		if err != nil {
			return nil, fmt.Errorf("manager: open %q: %w", conn.Name, err)
		}
		m.drivers[conn.Name] = drv
	}
	return m, nil
}
...
func openDriver(conn config.Connection) (Driver, error) {
	...
	case "postgres":
		return postgres.New(dsn, postgres.PoolConfig{
			MaxOpen: conn.MaxConnections,
			MaxIdle: conn.MinConnections,
		})
	case "mysql":
	...
	default:
		return nil, fmt.Errorf("unsupported driver %q", conn.Engine)
	}
}
